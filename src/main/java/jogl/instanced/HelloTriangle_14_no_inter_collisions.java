 ...
 * Render stuff things
 */
public class HelloTriangle_14_no_inter_collisions extends HelloTriangle_Base implements KeyListener {
    ...
    public HelloTriangle_14_no_inter_collisions() {
        w.setGravity(0, 0);
    }

    public static void main(String[] args) {
        HelloTriangle_14_no_inter_collisions gui = new HelloTriangle_14_no_inter_collisions();
        gui.setup(gui);
    }
    ...
    private static Transform buildTransform(double x, double y, float a, float scale){
        return new Transform(new Vector2(x, y), a, scale);
    }

    private static Transform buildTransform(double x, double y, float a){
        return buildTransform(x, y, a, 1);
    }

    private static Transform buildZeroTransform(){
        return buildTransform(0, 0, 0);
    }
    ...
    private static void addModelsToEntity(Entity e, List<Pair<Model, Transform>> models){
        for (Pair<Model, Transform> pair : models) {
            ...
            for (int i = 0; i < vertices.length; i++) {
                vertices[i] = pair.component1().asVectorData[i].copy().multiply(pair.component2().scale);
            }
...

    {
        ...
        for (int i = 0; i < count; i++) {
            Entity e = new Entity();
            float scale = (float)Math.random()/2f + 0.1f;
            addModelsToEntity(e, List.of(
                    new Pair<>(Model.TRIANGLE, buildTransform(0.0d, 0.0d, 0, scale)),
                    new Pair<>(Model.SQUARE1, buildTransform(-0.5d*scale, -1.5d*scale, 0, scale)),
                    new Pair<>(Model.SQUARE2, buildTransform(0.5d*scale, -1.5d*scale, 0, scale))
            ));

            e.translate((i % scaling)/4f, Math.floor(i/(float)scaling)/4f);
            entities.add(e);
            w.addBody(e);
            e.applyForce(new Vector2(Math.random(), Math.random()));
        }
    ...
    private static class Entity extends Body {
        ...
        public List<Pair<Model, Transform>> getTransformedComponents(){
            ...
            for (Pair<Model, Transform> component : models) {
                ...
                result.add(new Pair<>(component.component1(),new Transform(newPos, newAngle, component.component2().scale)));
            }
    ...
    private record Transform(Vector2 position, float angle, float scale){}
    ...
    private void initProgram(GL3 gl) {
        ...
        program = new Program(gl, getClass(), "", "hello-triangle_13", "hello-triangle_13");

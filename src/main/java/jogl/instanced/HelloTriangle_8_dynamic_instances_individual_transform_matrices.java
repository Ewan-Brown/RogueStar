 ...
 *
 */
public class HelloTriangle_8_dynamic_instances_individual_transform_matrices extends HelloTriangle_Base {

    public static void main(String[] args) {
        System.err.println("Probably not working.");
        HelloTriangle_8_dynamic_instances_individual_transform_matrices gui = new HelloTriangle_8_dynamic_instances_individual_transform_matrices();
        gui.setup();
    }
    ...
    static final int MODEL_COUNT = 1;
    ...
    private void initVAOs(GL3 gl) {
        ...
        gl.glBindVertexArray(VAOs.get(0));
        {
            ...
            //https://stackoverflow.com/a/17355139/5175581
            gl.glBindBuffer(GL_ARRAY_BUFFER, VBOs.get(Buffer.INSTANCED_STUFF));
            for (int i = 0; i < 4; i++) {
                gl.glEnableVertexAttribArray(INSTANCE_POSITION_ATTRIB_INDICE + i);
                gl.glVertexAttribPointer(INSTANCE_POSITION_ATTRIB_INDICE + i, 4, GL_FLOAT, false, 16 * Float.BYTES, i * Float.BYTES * 4);
                gl.glVertexAttribDivisor(INSTANCE_POSITION_ATTRIB_INDICE + i, 1);
            }
    ...
    Vector2[] positions = new Vector2[MODEL_COUNT];
    float[] angles = new float[MODEL_COUNT];
    ...
    private void updateInstanceData(GL3 gl){
        FloatBuffer instanceBuffer = GLBuffers.newDirectFloatBuffer(MODEL_COUNT * 16);
...
//            float[] translation = FloatUtil.makeTranslation(new float[16], true, 0, 0, 0);
//            float[] zRotation = FloatUtil.makeRotationEuler(new float[16], 0, 0, 0, 0);
//            float[] transformMatrix = FloatUtil.multMatrix(translation, zRotation);

            float[] transformMatrix = FloatUtil.makeIdentity(new float[16]);
//            float[] transformMatrix = new float[16];

            instanceBuffer.put(transformMatrix);
...
//            angles[i] += (i % 2 == 0) ? -(0.3f/i) : (0.3f/i);
    ...
    private void initProgram(GL3 gl) {
        ...
        program = new Program(gl, getClass(), "", "hello-triangle_8", "hello-triangle_8");
...
//        gl.glPointSize(10f);
        gl.glDrawArraysInstanced(GL_TRIANGLES, 0,  3, MODEL_COUNT);
//        gl.glDrawArraysInstanced(GL_TRIANGLE_FAN, 3,  4, MODEL_COUNT);
        gl.glUseProgram(0);
        gl.glBindVertexArray(0);

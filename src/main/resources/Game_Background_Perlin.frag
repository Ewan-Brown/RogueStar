...
float color(vec2 xy) { return snoise(vec3(xy, 0.3*time)); }
...
void main()
{
    ...
    //Gives illusion of going 'forwards' ? "Stretching" image..
    float throttle = sin(time*2)*0.3 + 1;

    vec3 xyz = vec3(pos.x, pos.y, 0)*throttle;
    ...
    float b = colorCalculated(xyz, time+100)+0.2;


    outputColor.xyz = vec3(a*b, a*b, a*b);
